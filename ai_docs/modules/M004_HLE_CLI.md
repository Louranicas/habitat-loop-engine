...
- `hle run`, `hle verify`, and `hle daemon --once` are local-only;
- `hle verify` decodes escaped JSON receipt string fields before verifier aggregation;
- module listed in `plan.toml`;
- module listed in `ULTRAMAP.md`;
- unbounded daemon operation is rejected.
...
- Previous authority: `L06_CLI.md` in `ai_docs/layers/` plus `ULTRAMAP.md`.
- Source authority: `crates/hle-cli/src/main.rs`.
...
- Current file: `ai_docs/modules/M004_HLE_CLI.md`.
- Parent directory: `ai_docs/modules`.
- Adjacent markdown siblings sampled: INDEX.md, M001_SUBSTRATE_TYPES.md, M002_SUBSTRATE_VERIFY.md, M003_SUBSTRATE_EMIT.md.
