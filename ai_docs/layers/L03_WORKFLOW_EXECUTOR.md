...
- Define boundaries for L03.
- Execute local one-shot workflow steps only through bounded foreground commands.
- Reject live-integration/network tokens and blocked command basenames, including absolute paths such as `/bin/rm` and separator-chained tokens such as `safe;rm`.
...
- Downstream module authority: `M003_SUBSTRATE_EMIT` in `ai_docs/modules/`.
- Downstream source authority: `crates/substrate-emit/src/lib.rs`.
...
Crates: substrate-emit, hle-executor.

Clusters touching this layer (per `ai_docs/CLUSTERED_MODULES.md`):

- C02 (M012 state_machine, M013 status_transitions), C03 (M015-M019 bounded/local_runner/phase_executor/timeout_policy/retry_policy)
...
- Current file: `ai_docs/layers/L03_WORKFLOW_EXECUTOR.md`.
- Parent directory: `ai_docs/layers`.
- Adjacent markdown siblings sampled: L01_FOUNDATION.md, L02_PERSISTENCE.md, L04_VERIFICATION.md, L05_DISPATCH_BRIDGES.md, L06_CLI.md, L07_RUNBOOK_SEMANTICS.md.
