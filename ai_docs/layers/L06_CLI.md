...
- Define boundaries for L06.
- Expose bounded `hle run`, `hle verify`, and `hle daemon --once` commands.
- Decode escaped JSON receipt fields during local verification before verifier aggregation.
...
- Downstream module authority: `M004_HLE_CLI` in `ai_docs/modules/`.
- Downstream source authority: `crates/hle-cli/src/main.rs`.
...
Crates: hle-cli (existing + extended), scripts/.

Clusters touching this layer (per `ai_docs/CLUSTERED_MODULES.md`):

- C08 (M046-M050 cli_args/cli_run/cli_verify/cli_daemon_once/cli_status), C09 (M051-M054 verify_sync_script/quality_gate_script/module_map_script/layer_dag_script)
...
- Current file: `ai_docs/layers/L06_CLI.md`.
- Parent directory: `ai_docs/layers`.
- Adjacent markdown siblings sampled: L01_FOUNDATION.md, L02_PERSISTENCE.md, L03_WORKFLOW_EXECUTOR.md, L04_VERIFICATION.md, L05_DISPATCH_BRIDGES.md, L07_RUNBOOK_SEMANTICS.md.
