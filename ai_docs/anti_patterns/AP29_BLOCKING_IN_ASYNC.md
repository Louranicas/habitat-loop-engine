...
A future detector for `AP29_BLOCKING_IN_ASYNC` must identify evidence of blocking work inside async execution contexts without async-native APIs or explicit blocking isolation in source, configuration, verifier receipts, or scaffold review artifacts. The detector predicate is reviewable at scaffold time even though executable runtime detection is not implemented here.
...
A remediation receipt must name the affected file, describe the semantic correction, and point to verifier evidence. Count-only evidence such as file presence or registry size is insufficient for `AP29_BLOCKING_IN_ASYNC`.
...
- Current file: `ai_docs/anti_patterns/AP29_BLOCKING_IN_ASYNC.md`.
- Parent directory: `ai_docs/anti_patterns`.
- Adjacent markdown siblings sampled: AP28_COMPOSITIONAL_INTEGRITY_DRIFT.md, AP31_NESTED_LOCKS.md, C12_UNBOUNDED_COLLECTIONS.md, C13_MISSING_BUILDER.md, C6_LOCK_HELD_SIGNAL_EMIT.md, C7_LOCK_GUARD_REFERENCE_RETURN.md, FP_FALSE_PASS_CLASSES.md, INDEX.md.
