...
A future detector for `C7_LOCK_GUARD_REFERENCE_RETURN` must identify evidence of returning references tied to lock guards beyond the guard lifetime boundary in source, configuration, verifier receipts, or scaffold review artifacts. The detector predicate is reviewable at scaffold time even though executable runtime detection is not implemented here.
...
A remediation receipt must name the affected file, describe the semantic correction, and point to verifier evidence. Count-only evidence such as file presence or registry size is insufficient for `C7_LOCK_GUARD_REFERENCE_RETURN`.
...
- Current file: `ai_docs/anti_patterns/C7_LOCK_GUARD_REFERENCE_RETURN.md`.
- Parent directory: `ai_docs/anti_patterns`.
- Adjacent markdown siblings sampled: AP28_COMPOSITIONAL_INTEGRITY_DRIFT.md, AP29_BLOCKING_IN_ASYNC.md, AP31_NESTED_LOCKS.md, C12_UNBOUNDED_COLLECTIONS.md, C13_MISSING_BUILDER.md, C6_LOCK_HELD_SIGNAL_EMIT.md, FP_FALSE_PASS_CLASSES.md, INDEX.md.
