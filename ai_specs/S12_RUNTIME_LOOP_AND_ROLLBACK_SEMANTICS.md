# S12 RUNTIME LOOP AND ROLLBACK SEMANTICS

Status: M0 local implementation surface.

## Authority

The runtime loop is bounded and local. The verifier remains the sole PASS/FAIL authority, and awaiting-human states stop the loop before external mutation.

## Acceptance

- Present in `ai_specs/INDEX.md`.
- Local run stops on failed or awaiting-human receipts.
- Rollback semantics remain receipt-driven; no live rollback integrations are installed.
- Referenced by scaffold verification.
- Not marked execution PASS before implementation evidence exists.
- Cron jobs and bounded daemon-once operation are allowed only with explicit M0/deployment authorization and receipt evidence.
...
- Current file: `ai_specs/S12_RUNTIME_LOOP_AND_ROLLBACK_SEMANTICS.md`.
- Parent directory: `ai_specs`.
- Adjacent markdown siblings sampled: INDEX.md, S01_SYSTEM_OVERVIEW_AND_AUTHORITY_BOUNDARY.md, S02_WORKFLOW_DEFINITION_MODEL.md, S03_EXECUTOR_STATE_MACHINE.md, S04_VERIFIER_AND_RECEIPT_AUTHORITY.md, S05_PERSISTENCE_LEDGER_AND_SQLITE_SCHEMA.md, S06_CLI_AND_LOCAL_OPERATION_SURFACE.md, S07_ANTI_USE_PATTERN_INTELLIGENCE.md.
