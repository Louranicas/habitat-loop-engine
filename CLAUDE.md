...
This repository is authorized for bounded M0 local implementation; live integrations and unbounded daemon behavior remain forbidden. The codebase needs to be 'one shotted': every runtime operation must run once, foreground, bounded, and verifier-visible.

## Rules

- Keep runtime executor behavior bounded to local one-shot M0 commands; no persistent loop may be introduced without a later explicit authorization receipt.
- Do not create live Habitat write integrations.
- Do not create cron jobs, services, or unbounded daemons.
...
- Parent directory: `.`.
- Adjacent markdown siblings sampled: ARCHITECTURE.md, CHANGELOG.md, CLAUDE.local.md, CODEOWNERS.md, HARNESS_CONTRACT.md, QUALITY_BAR.md, QUICKSTART.md, README.md.
